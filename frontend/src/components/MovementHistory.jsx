...
export default function MovementHistory({ movements, selectedProduct, filters, onFilterChange, onApplyFilters }) {
  ...
  useEffect(() => {
    if (selectedProduct) setIsOpen(true);
  }, [selectedProduct]);
  ...
  return (
    <section className={`card movement-history ${isOpen ? 'movement-history--open' : ''}`}>
      <button
        type="button"
        className="movement-history__header"
        aria-expanded={isOpen}
        aria-controls="movement-history-panel"
        onClick={() => setIsOpen((current) => !current)}
      >
        <span className="movement-history__title">Historial de Movimientos</span>
        {isOpen ? <ChevronUp size={20} aria-hidden="true" /> : <ChevronDown size={20} aria-hidden="true" />}
      </button>

      <div
        id="movement-history-panel"
        className="movement-history__panel"
        aria-hidden={!isOpen}
